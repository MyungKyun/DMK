...
DBConnection::DBConnection()
{
	...
	::swprintf_s(buf.data(), buf.size(), L"Driver={SQL Server};Server=%s,%s;DataBase=%s;Uid=%s;Pwd=%s", Server.c_str(), port.c_str(), db.c_str(), uid.c_str(), pwd.c_str());
...
Bool DBConnection::Connect(SQLHENV henv)
{
	...
	if (false == DBErrorMessage::CheckError(SQL_HANDLE_ENV, henv, ret))
	{
		Disconnect();
		return false;
	}
	...
	if (false == DBErrorMessage::CheckError(SQL_HANDLE_DBC, hdbc_, ret))
	{
		Disconnect();
		return false;
	}
